...
class multitridentMultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        for idx in range(num):
            ...
            if self.use_ARM:
                matches = refine_match_return_matches(self.threshold, truths, defaults, self.variance, labels,
        ...
        loss_c_for_small = log_sum_exp(batch_conf) - batch_conf.gather(1, conf_t.view(-1, 1))
        batch_conf = conf_data2.view(-1, self.num_classes)
        loss_c_for_middle = log_sum_exp(batch_conf) - batch_conf.gather(1, conf_t.view(-1, 1))
        batch_conf = conf_data3.view(-1, self.num_classes)
        loss_c_for_big = log_sum_exp(batch_conf) - batch_conf.gather(1, conf_t.view(-1, 1))
        #print(loss_c.size())
